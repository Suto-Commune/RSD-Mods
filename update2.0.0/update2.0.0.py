...
for i in range(0, latest_file_num):
    ...
    if os.path.exists('./.minecraft/mods/' + str(latest_file_name[i])):
        path = './.minecraft/mods/' + str(latest_file_name[i])
...
for i in range(0, len(latest_file_will_do)):
    if latest_file_will_do[i] == 1 or latest_file_will_do[i] == 3:
        ...
        download("https://redstone-download.netlify.app/" + str(latest_file_name[i]),
                 "./.minecraft/mods/" + str(latest_file_name[i]))
        zhuangtai += 1
    elif latest_file_will_do[i] == 2 and os.path.exists("./.minecraft/mods/" + str(latest_file_name[i])):
        print("发现冗余Mod(s): " + str(latest_file_name[i]) + " ,即将删除")
        # if os.path.exists("./.minecraft/mods/"+latest_file_name[i]):
        os.remove("./.minecraft/mods/" + str(latest_file_name[i]))
        zhuangtai += 1
if zhuangtai == 0:
